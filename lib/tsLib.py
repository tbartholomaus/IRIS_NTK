...
from obspy.io.stationxml.core import _is_stationxml as is_StationXML
...
#
# HISTORY:
#    2017-01-18 Manoch: made the following changes to avoid ObsPy deprecation warnings:
#                   < from obspy.station.stationxml import is_StationXML,validate_StationXML
#                   ---
#                   > from obspy.io.stationxml.core import _is_stationxml as is_StationXML
#                   > from obspy.io.stationxml.core import validate_stationxml as validate_StationXML
...
###########################################################
#
def getResponseInventory(respdir, debug = False):
   t0          = time()
   acceptCount = 0
   rejectCount = 0
   print("[INFO] reading response files")
   fileList = [f for f in listdir(respdir) if isfile(join(respdir, f))]
   inventory = []
   for respFile in fileList:
      try:
         validation = validate_StationXML(os.path.join(respdir, respFile))
         if ( not validation[0] ):
             print("[ERR] Skipped invalid response file",os.path.join(respdir,respFile))
             print("[ERR]",str(validation[1]))
         inv = read_inventory(os.path.join(respdir, respFile))
         acceptCount += 1
         ...
         if debug:
            print("[ERR] Skipped, could not read",os.path.join(respdir,respFile))
         rejectCount += 1
   t1 = fileLib.timeIt("Response Inventory",t0)
   print("[INFO] response inventory:",acceptCount,"valid",rejectCount,"rejected")
   return inventory
...
###########################################################
#
def getResponseFromFile  (inventory, respdir, network, station, location, channel, starttime, debug):
   #
   # if inventory list is not populated, populate it first
   #
   if inventory is None:
      inventory = getResponseInventory(respdir, False)

   response = None
   thisStartTime = UTCDateTime(starttime)
   seedId  = ".".join([network,station,location,channel])
   print("[INFO] looking for seed ID:",seedId)
   thisInventory = None
   for inv in inventory:
     try:
       response = inv.get_response(seedId, thisStartTime)
       if debug:
       ...
       response = None 
   return inventory, thisInventory
