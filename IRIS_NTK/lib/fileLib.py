...
def makePath(directory):
    ...
    if not os.path.isabs(directory):
        print('ERR [checkMakePath]: path must be an absolute path')
        return None
...
################################################################################################
#
def getDir(dataDirectory,subDirectory,network,station,location,channel=None):
   if channel is None:
      directoryTag  = os.path.join(dataDirectory,subDirectory, getTag(".",[network,station,location]))
      fileTag       = getTag(".",[network,station,location])
   else:
