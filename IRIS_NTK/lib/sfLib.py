...
def smoothAngleF (frequency,power,samplingRate,octaveWindowWidth,octaveWindowShift,minFrequency,xStart,rotation):
   #
   ##########################################################################
   ...
   #
   # HISTORY:
   #    2015-06-01 Manoch: made the start frequency selectable to support Nyquist or 1 as the start frequency, like smoothF
   #    2014-02-07 Manoch: created
   # 
   #
   ##########################################################################
...
def smoothAngleNyquist(type,Xi,Yi,samplingRate,octaveWindowWidth,octaveWindowShift,xLimit,rotation):
   #
   ##########################################################################
   ...
   while (True):
      ...
      if (len(thisBin) > 0):
         ...
         # the correct quadrant for the angle. For example, atan(1) and atan2(1, 1) are both pi/4, but atan2(-1, -1) is -3*pi/4.
         #
         Y.append(rotation -(math.atan2(meanSin,meanCos)*180.0/math.pi))
         X.append(Xc)
...
def smoothAngleT(period,power,samplingRate,octaveWindowWidth,octaveWindowShift,maxPeriod,pStart,rotation):
   #
   ##########################################################################
   # smooth the spectra in the period domain
   #
   ...
   #          wanted to be consistent in our calculations
   # HISTORY:
   #    2015-06-01 Manoch: made the start period selectable to support Nyquist or 1 as the start period, like smoothT
   #    2014-02-07 Manoch: created
   # 
   #
   ##########################################################################
