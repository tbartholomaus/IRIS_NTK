...
# HISTORY:
#
#    2020-09-25 Bartholomaus: R 0.7.0 conversion to python 3
...
# LOOP: WINDOW
#
for tStep in range(0,duration,msgLib.param(param,'windowShift').windowShift):
   ...
   for streamIndex in qcRecords:
     ...
     if msgLib.param(param,'performInstrumentCorrection').performInstrumentCorrection:
         powerUnits = msgLib.param(param,'powerUnits').powerUnits[trChannel1.stats.response.instrument_sensitivity.input_units.upper()]
     else:
     ...
     nSegCount = 0
     for n in range(0,msgLib.param(param,'nSegments').nSegments):
        if TIMING:
     ...
     if xType == "period":
        ...
        #
        period    = np.append([10.0*msgLib.param(param,'maxT').maxT],1.0/(np.arange(1.0,spec_length)/float(nSamp * delta)))
        if param.doSmoothing:
        ...
        frequency = np.array( np.arange(0,spec_length)/float(nSamp * delta))
